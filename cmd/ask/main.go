...
type IncomingMessage struct {
	...
	SamlAssert   string  `json:"samlAssert"`
...
func main() {
	// check for message
	file, err := os.OpenFile("ask-log.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
...
func generateKeys(roleArn string, principalArn string, samlAssert string) *sts.AssumeRoleWithSAMLOutput {
	...
	input := &sts.AssumeRoleWithSAMLInput{
		...
		SAMLAssertion:   aws.String(samlAssert),
	}
...
func updateKeys(iMsg IncomingMessage) {
	...
	p.Set(iMsg.Options.ProfileName, "region", iMsg.Options.Region)
	p.Set(iMsg.Options.ProfileName, "aws_access_key_id", *keys.Credentials.AccessKeyId)
	p.Set(iMsg.Options.ProfileName, "aws_secret_access_key", *keys.Credentials.SecretAccessKey)
